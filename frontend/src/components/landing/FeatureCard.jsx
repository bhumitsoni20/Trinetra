...
export default function FeatureCard({ icon: Icon, title, description, tone = "cyan", tag }) {
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:scale-[1.01] sm:p-6 ${toneStyle.border}`}
    >
      <div
      ...
      {tag ? (
        <span className="mb-3 inline-flex rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.16em] text-slate-300 sm:mb-4 sm:py-1 sm:text-[11px]">
          {tag}
        </span>
      ) : null}
      ...
      <div className="relative z-10">
        ...
        <p className="mt-2 text-xs leading-relaxed text-slate-400 sm:mt-3 sm:text-sm">{description}</p>
      </div>
    </article>
  );
}
